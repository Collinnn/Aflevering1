...
plot(Air$temperature, Air$logOzone, xlab="Temperature", ylab="Ozone")
## Correlation
...
plot(Air$noise, Air$logOzone, ylab="Ozone", xlab="Noise")
...
wallyplot(fitSel, FUN=qqnorm.wally, main="")
...
fitWindSqTemperaturSq <- lm(logOzone ~ temperature + temperatureSq + wind + windSq + radiation, data=Air)
...
## legend
legend("topright", cex = 0.8, c("Prediction","95% confidence band","95% prediction band"), lty=c(1,2,2), col=1:3)
...
par(mfrow = c(1,2))
plot(x1, type="b", ylab = 'x1(black), x2 (red)')
